...
import com.dam.trabajopmdm.sqlite.FeedReaderContract
...
class ControladorSQLite(context: Context) {
    ...
    fun guardarRecord(puntuacion: Int, fecha: Long) {
        ...
        Log.d(
            "SQLite",
            "INSERT: Record guardado correctamente. ID fila: $newRowId. Score: $puntuacion"
        )
    }
    ...
    fun obtenerDatos(): List<String> {
        ...
        db.query(
            ...
            while (cursor.moveToNext()) {
                ...
                val score =
                    cursor.getInt(cursor.getColumnIndexOrThrow(FeedReaderContract.FeedEntry.COLUMN_NAME_SCORE))
                val dateLong =
                    cursor.getLong(cursor.getColumnIndexOrThrow(FeedReaderContract.FeedEntry.COLUMN_NAME_DATE))
    ...
    fun actualizarUltimoRecord(nuevaPuntuacion: Int) {
        ...
        val queryMaxDate =
            "SELECT MAX(${FeedReaderContract.FeedEntry.COLUMN_NAME_DATE}) FROM ${FeedReaderContract.FeedEntry.TABLE_NAME}"
        ...
        if (fechaMasReciente != null) {
            ...
            Log.d(
                "SQLite",
                "UPDATE: Se actualizó el último registro (Fecha: $fechaMasReciente). Filas afectadas: $count. Nuevo Score: $nuevaPuntuacion"
            )
        } else {
